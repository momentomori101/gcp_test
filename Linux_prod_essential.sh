...
#!/bin/bash backup_dir="/path/to/backup" 
...
#!/bin/bash threshold=90 cpu_usage=$(top -bn1 | grep "Cpu(s)" | awk '{print $2}' | cut -d. -f1) if [ "$cpu_usage" -gt "$threshold" ]; then     echo "High CPU usage detected: 
$cpu_usage%" 
# Add alert/notification logic here fi 
...
if id "$username" &>/dev/null; then     echo "User $username already exists." else     useradd -m "$username" 
...
#!/bin/bash file="/path/to/file.txt" openssl enc -aes-256-cbc -salt -in "$file" -out 
"$file.enc" 
...
if ping -c 1 "$host" &>/dev/null; then     echo "Network is up." else 
    echo "Network is down." fi 
...
"$website"; then     echo "Website is up." else 
...
#12.	System Information Script: #!/bin/bash 
output_file="system_info.txt" 
 
echo "System Information:" > 
"$output_file" echo "-------------------" >> "$output_file" echo "Hostname: $(hostname)" >> 
"$output_file" 
...
disk_usage=$(df -h | grep "/dev/sda1" | awk '{print $5}' | cut -d% -f1) if [ "$disk_usage" -gt "$threshold" ]; then     echo "High disk usage detected: 
$disk_usage%" 
    # Add alert/notification logic here fi 
...
#!/bin/bash 
source_dir="/path/to/source" remote_server="user@remoteserver:/path/to/backup" 
...
#!/bin/bash database_name="your_database" output_file="database_backup_$(date 
+%Y%m%d).sql" 
 
mysqldump -u username -ppassword "$database_name" > "$output_file" echo "Database backup created: 
$output_file" 
 
#19.	Git Repository Updater Script: #!/bin/bash 
git_repo="/path/to/your/repo" 
 
cd "$git_repo" git pull origin master echo "Git repository updated." 
 
#20.	Directory Synchronization Script: 
 
#!/bin/bash 
source_dir="/path/to/source" destination_dir="/path/to/destination" 
...
#!/bin/bash 
log_file="/var/log/apache2/access.log" awk '{print $1}' "$log_file" | sort | uniq -c | sort -nr 
...
"$database_name*.sql" -mtime +"$days_to_keep" -exec rm {} \; echo "Old database backups cleaned up." 
...
"/bin/bash" | cut -d: -f1); do     password_expires=$(chage -l "$user" | grep "Password expires" | awk '{print $4}')     echo "User: $user, Password Expires: 
$password_expires" done unset IFS 
...
#!/bin/bash 
remote_server="user@remote-server" remote_script="/path/to/remote/script.sh
" 
...
ifconfig "$network_interface" echo "Network interface information 
displayed." 
...
random_index=$((RANDOM % 
${#quotes[@]})) echo "Random Quote: ${quotes[$random_index]}" 
